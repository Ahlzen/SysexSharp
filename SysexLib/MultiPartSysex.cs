...
/// </summary>
public class MultiPartSysex : Sysex
{
    protected List<Sysex> Sysexes { get; } = new();

    public int ItemCount => Sysexes.Count;
    public Sysex GetSysex(int index) => Sysexes[index];
    public IEnumerable<Sysex> GetSysexes() => Sysexes;
    public IEnumerable<string>? GetSysexNames() => Sysexes.Select(s => s.Name ?? "");
    ...
    /// </remarks>
    public MultiPartSysex(byte[] data, string? filename = null) : base(data, filename)
    {
        int sysexIndex = 0;
        int start = -1;
    ...
    public static MultiPartSysex FromSysexes(IEnumerable<Sysex> sysexes, string? filename)
    {
        var data = new List<byte>();
        foreach (Sysex sysex in sysexes)
            data.AddRange(sysex.GetData());
        return new MultiPartSysex(data.ToArray(), filename);
    }
