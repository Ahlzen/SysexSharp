...
namespace Ahlzen.SysexSharp.SysexLib.Manufacturers.Yamaha
{
    ...
    /// </remarks>
    public class TX81ZVoiceBank : DX_TX_Bank
    {
        ...
        protected override byte?[] Header => DX_TX_Data.DX21_TX81Z_BankHeader; //DXData.TX81ZBankHeader;

        protected override List<Parameter> Parameters => DX_TX_Data.TX81ZPackedVoiceDataParameters;
        protected override Dictionary<string, Parameter> ParametersByName => DX_TX_Data.TX81ZPackedVoiceDataParametersByName;

        public override int ItemCount => 32;
        protected override int ItemSize => DX_TX_Data.TX81ZPackedVoiceDataSize;
        protected override string? ItemNameParameter => "Voice name";

        public static bool Test(byte[] data)
        {
            if (!Sysex.Test(data)) return false;
            if (data.Length != DX_TX_Data.TX81ZBankSize) return false;
            ...
            for (int voice = 0; voice < 32; voice++) {
                int voiceOffset = 6 + DX_TX_Data.TX81ZPackedVoiceDataSize * voice;
                ...
                if (!ParsingUtils.MatchesPattern(data,
                    DX_TX_Data.TX81ZUnusedGlobalPitchEGData, voiceOffset + 67))
                        return false;
